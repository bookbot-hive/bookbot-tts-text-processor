...
def generate_inputids(text, language, anim_path):
    ...
    if language not in models:
        models[language] = TextProcessor(model_dirs[language], db_paths[language], language=language, use_cosmos=False, cosmos_config=cosmos_config, animation_tags_path=anim_path)     
    model = models[language]
    return model.get_input_ids(text, phonemes=False, return_phonemes=True, push_oov_to_cosmos=False, add_blank_token=True)
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_json_path):
        ...
        new_items = [item for item in input_data if item.get("hash") not in existing_hashes]
        ...
        for item in new_items:
            ...
            try:
                ...
                print(f"Processed item: hash '{hash_value}', language '{language}', input_ids '{input_ids}'")
    ...
    if error_list:
        ...
        raise Exception(f"Processing completed with errors. See {error_log_path} for details")
